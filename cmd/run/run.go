...
This command allows you to run a specific command within the moat environment.`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			env, err := config.GetEnv(config.CmdConfig, envName, true)
			if err != nil {
			...
			if len(command) == 0 {
				if env.Command == nil {
					log.Error().Msgf("No command provided and no default command set in the environment")
					return
				}
			...
			if env.Runtime != nil {
				...
				runtimeName = config.CmdConfig.GetString("defaults.runtime")
			}
