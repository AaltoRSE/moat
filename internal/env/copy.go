...
// source's mounts. If roMounts is non-empty, it overrides the source's
// read-only mounts. If command is non-empty, it overrides the source's
// command. All other fields of the source environment are preserved.
// autoCreate has the same meaning as in [CreateEnvironment].
func CopyEnvironment(cfg *viper.Viper, sourceName, newName, home string, mounts []string, roMounts []string, command *string, autoCreate bool) error {
	...
	if len(roMounts) > 0 {
		source.ReadOnlyMounts = roMounts
	}
	if command != nil {
